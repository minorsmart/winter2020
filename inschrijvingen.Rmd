...
dfOpleidingType <- data.frame(Type = colnames(tblOpleidingType)[1:2], Aantal = tblOpleidingType[dim(tblOpleidingType)[1],1:2])
...
```{r echo=FALSE, warning=FALSE}
dfOpleidingNaam <- data.frame(Naam = colnames(tblOpleidingNaam)[1:10], Aantal = tblOpleidingNaam[dim(tblOpleidingNaam)[1],1:10])
